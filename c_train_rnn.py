...
def train(experiment, model, config, data_iterator, tr, logger):
    ...
    for step in range(tr.updates_done, config.max_steps):
        ...
        for i in range(0, data["datalen"]):
            ...
            x = numpy.asarray(data['x'][i]).astype("float32")
            y = numpy.asarray(one_hot(data['y'][i], numd)).astype("float32")
        ...
        W_grad = np.linalg.norm(model._layer_list[0]._updates['W'])
        ...
        U_grad = np.linalg.norm(model._layer_list[0]._updates['U'])
