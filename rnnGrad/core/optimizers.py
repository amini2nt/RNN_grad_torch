            ...
            if len(layer._params) > 0:
                if layer.is_time_shared():
                    for param in layer._params.keys():
                        ...
                        layer._updates[param] = -(
                                self._lr * grad / (torch.sqrt(self._params["sqrs"][i][param]) + self._eps))
                        layer._params[param] = layer._params[param] + layer._updates[param]
                ...
                if layer.is_time_shared():
                    for param in layer._params.keys():
                        ...
                        for t in layer._grads.keys():
                            if t <= time:
                                grad += layer._grads[t][param]
                        self._params["g2"][i][param] = self._alpha * self._params["g2"][i][param] + (
                                1 - self._alpha) * (grad ** 2)
                        layer._updates[param] = - (
                                self._lr / (torch.sqrt(self._params["g2"][i][param]) + self._eps)) * grad
                        layer._params[param] = layer._params[param] + layer._updates[param]
                else:
                    for param in layer._params.keys():
                        grad = layer._grads[time][param]
                ...
                if layer.is_time_shared():
                    for param in layer._params.keys():
                        ...
                        for t in layer._grads.keys():
                            if t <= time:
                                grad += layer._grads[t][param]
                        self._params["sqr"][i][param] = self._gamma * self._params["sqr"][i][param] + (
                                1 - self._gamma) * (grad ** 2)
                        layer._updates[param] = - (torch.sqrt(self._params["delta"][i][param] + self._eps) / torch.sqrt(
                            self._params["sqr"][i][param] + self._eps)) * grad
                        self._params["delta"][i][param] = self._gamma * self._params["delta"][i][param] + (
                                1 - self._gamma) * layer._updates[param] * layer._updates[param]
            ...
            if len(layer._params) > 0:
                if layer.is_recurrent():
                    for param in layer._params.keys():
                        ...
                        self._params["m"][i][param] = self._beta1 * self._params["m"][i][param] + (1 - self._beta1) * (
                            grad)
                        self._params["v"][i][param] = self._beta2 * self._params["v"][i][param] + (1 - self._beta2) * (
                                grad ** 2)
                        ...
                        layer._updates[param] = - (step_size * self._params["m"][i][param] / (
                                torch.sqrt(self._params["v"][i][param]) + self._eps))
                        layer._params[param] = layer._params[param] + layer._updates[param]
                else:
                    for param in layer._params.keys():
                        grad = layer._grads[time][param]
...
#############################
# Weight sharing aware optimizers.
#############################

class WA_ADAGRAD(Optimizer):
    """Implementation of WA_ADAGRAD update rule."""

    def __init__(self, learning_rate=0.01, eps=1.0e-8):
        """Initialize an WA_ADAGRAD optimizer.
...
class WA_RMSprop(Optimizer):
    """Implementation of WA_RMSprop update rule."""

    def __init__(self, learning_rate=0.01, alpha=0.99, eps=1.0e-8):
        """Initialize an WA_RMSprop optimizer.
...
class WA_ADADELTA(Optimizer):
    """Implementation of WA_ADADELTA update rule."""

    def __init__(self, learning_rate=1.0, gamma=0.9, eps=1.0e-8):
        """Initialize an WA_ADADELTA optimizer.
                    ...
                    for param in layer._params.keys():
                        ...
                        for t in layer._grads.keys():
                            if t <= time:
                                ...
                                self._params["sqr"][i][param] = self._gamma * self._params["sqr"][i][param] + (
                                        1 - self._gamma) * (grad ** 2)
                                update = - (torch.sqrt(self._params["delta"][i][param] + self._eps) / torch.sqrt(
                                    self._params["sqr"][i][param] + self._eps)) * grad
                                self._params["delta"][i][param] = self._gamma * self._params["delta"][i][param] + (
                                        1 - self._gamma) * update * update
                                layer._updates[param] += self._lr * update
                        layer._params[param] = layer._params[param] + layer._updates[param]
...
class WA_ADAM(Optimizer):
    """Implementation of WA_ADAM update rule."""

    def __init__(self, learning_rate=0.01, beta1=0.9, beta2=0.999, eps=1.0e-8):
        """Initialize an WA_ADAM optimizer.
                    ...
                    for param in layer._params.keys():
                        ...
                        for t in layer._grads.keys():
                            if t <= time:
                                ...
                                self._params["m"][i][param] = self._beta1 * self._params["m"][i][param] + (
                                        1 - self._beta1) * (grad)
                                self._params["v"][i][param] = self._beta2 * self._params["v"][i][param] + (
                ...
                if layer.is_recurrent():
                    for param in layer._params.keys():
                        ...
                        for t in range(time, 0, -1):
                            ...
                            ta = time - t
                            # print(ta)
                            self._params["g2"][i][param][ta] = self._alpha * self._params["g2"][i][param][ta] + (
                                    1 - self._alpha) * (grad ** 2)
                            layer._updates[param] += - (self._lr / (np.sqrt(self._params["g2"][i][param][
                                                                                ta]) + self._eps)) * grad  ## the addition operation is not consistent

                        layer._params[param] = layer._params[param] + layer._updates[
                            param]  ## is the difference intentional
                else:
                    for param in layer._params.keys():
                        grad = layer._grads[time][param]
                        self._params["g2"][i][param] = self._alpha * self._params["g2"][i][param] + (
                                1 - self._alpha) * (grad ** 2)
                        layer._updates[param] = - (
                                self._lr / (np.sqrt(self._params["g2"][i][param]) + self._eps)) * grad
                        layer._params[param] = layer._params[param] + layer._updates[param]
                ...
                if layer.is_recurrent():
                    for param in layer._params.keys():
                        ...
                        for t in range(time, 0, -1):
                            grad = layer._grads[t][param]
                            ta = time - t
                            self._params["m"][i][param][ta] = self._beta1 * self._params["m"][i][param][ta] + (
                                    1 - self._beta1) * (grad)
                            self._params["v"][i][param][ta] = self._beta2 * self._params["v"][i][param][ta] + (
                                    1 - self._beta2) * (grad ** 2)
